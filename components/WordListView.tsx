      ...
      <div className="bg-white dark:bg-zinc-950 rounded-[2.5rem] shadow-xl overflow-hidden border-t-8 border-black dark:border-jsGold transition-colors">
        <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <button onClick={onBack} className="p-2 -ml-2 text-black dark:text-jsGold hover:scale-110 transition-transform"><ArrowLeft className="w-6 h-6" /></button>
        ...
        <div className="bg-zinc-50 dark:bg-zinc-900 p-4 border-b border-zinc-100 dark:border-zinc-800 overflow-x-auto no-scrollbar flex items-center gap-2">
           <button onClick={() => setSelectedLetter(null)} className={`px-5 py-2 shrink-0 rounded-full text-[10px] font-black transition-all ${!selectedLetter ? 'bg-black dark:bg-jsGold text-jsGold dark:text-black shadow-lg' : 'bg-white dark:bg-zinc-950 text-zinc-500 border border-zinc-200 dark:border-zinc-800 uppercase'}`}>ALL</button>
           {alphabet.map(l => (
                ...
                return (
                  <div key={i} className={`p-5 flex gap-4 transition-colors ${isSolved ? 'bg-green-50/20 dark:bg-green-900/10' : 'hover:bg-zinc-50 dark:hover:bg-zinc-900/50'}`}>
                    <div className="flex flex-col gap-3 pt-1">
                      ...
                      <button onClick={() => onToggleStar(word)} className={`transition-all ${isStarred ? 'text-jsGold fill-jsGold' : 'text-zinc-200 hover:text-jsGold'}`}>
